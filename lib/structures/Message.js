...
const { MessageFlags, InteractionTypes } = require("../Constants");
const User = require("./User");

function parseInteractionMetadata(client, data) {
  switch (data.type) {
    case InteractionTypes.APPLICATION_COMMAND: {
      return parseMetadataApplicationCommand(client, data);
    }
    case InteractionTypes.MESSAGE_COMPONENT: {
      return parseMetadataMessageComponent(client, data);
    }
    default: { // model submit
      return parseMetadataModelSubmit(client, data);
    }
  }
}

function parseBaseMetadata(client, data) {
  const baseMetadata = {
    authorizingIntegrationOwners: data.authorizing_integration_owners,
    id: data.id,
    originalResponseMessageID: data.original_response_message_id,
    type: data.type,
    user: client.users.update(data.user, client),
  };

  return baseMetadata;
}

function parseMetadataApplicationCommand(client, data) {
  const baseData = parseBaseMetadata(client, data);
  if (data.target_user) {
    baseData.targetUser = client.users.update(data.target_user, client);
  }
  baseData.targetMessageID = data.target_message_id;
  return baseData;
}

function parseMetadataMessageComponent(client, data) {
  const baseData = parseBaseMetadata(client, data);
  return {
    ...baseData,
    interactedMessageID: data.interacted_message_id,
  };
}

function parseMetadataModelSubmit(client, data) {
  const baseData = parseBaseMetadata(client, data);

  switch (data.triggering_interaction_metadata.type) {
    case InteractionTypes.APPLICATION_COMMAND:
      baseData.triggeringInteractionMetadata = parseMetadataApplicationCommand(client, data.triggering_interaction_metadata);
      break;
    default: // type message component
      baseData.triggeringInteractionMetadata = parseMetadataMessageComponent(client, data.triggering_interaction_metadata);
      break;
  }
 ...
 * @prop {Number?} call.endedTimestamp The time when the call end
 ...
 * @prop {Object?} interaction [DEPRECATED] An object containing info about the interaction the message is responding to, if applicable
 ...
 * @prop {Object?} interactionMetadata An object containing info about the interaction the message is responding to, if applicable
 * @prop {String} interactionMetadata.authorizingIntegrationOwners authorizingIntegrationOwners An object containing the values IDs of the user and/or guild that authorized the app. guildInstall for GuildID and userInstall for UserID
 * @prop {String} interactionMetadata.id The ID of the interaction that the message is replying to
 * @prop {string?} interactionMetadata.interactedMessageID ID of the message that contained the interactive component
 * @prop {String} interactionMetadata.originalResponseMessageID The ID of the original message, if applicable
 * @prop {Number} interactionMetadata.type The type of the interaction
 * @prop {Object?} interactionMetadata.targetMessageID The ID of the message the command was run on, present only on message command interactions.
 * @prop {Object?} interactionMetadata.targetUser The user the command was run on, present only on user command interaction
 * @prop {Object?} interactionMetadata.triggeringInteractionMetadata Metadata for the interaction that was used to open the modal, of type `MessageComponentInteractionMetadata` or `ApplicationCommandInteractionMetadata`;
 * @prop {Object} interactionMetadata.user The user who originally used the interaction
 * @prop {String} interactionMetadata.triggeringInteractionMetadata.id The ID of the interaction that the message is replying to
 * @prop {string?} interactionMetadata.triggeringInteractionMetadata.interactedMessageID ID of the message that contained the interactive component
 * @prop {Object} interactionMetadata.triggeringInteractionMetadata.user The user who originally used the interaction
 * @prop {Number} interactionMetadata.triggeringInteractionMetadata.type The type of the interaction
 * @prop {Object?} interactionMetadata.triggeringInteractionMetadata.targetMessageID The ID of the message the command was run on, present only on message command interactions.
 * @prop {Object?} interactionMetadata.triggeringInteractionMetadata.targetUser The user the command was run on, present only on user command interaction
...
class Message extends Base {
  constructor(data, client) {
    ...
    if (data.interaction_metadata) {
      this.interactionMetadata = parseInteractionMetadata(this._client, data.interaction_metadata);
    } else {
